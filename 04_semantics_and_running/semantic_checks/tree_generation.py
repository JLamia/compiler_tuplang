...
def p_function_definition_with_vars(p):
    '''function_definition_with_vars : DEFINE funcIDENT LSQUARE regexp_formals RSQUARE BEGIN regexp_variable_definition return_value DOT END DOT'''
...
def p_function_definition_empty(p):
    '''function_definition_empty : DEFINE funcIDENT LSQUARE regexp_formals RSQUARE BEGIN return_value DOT END DOT'''
    print("func_definition( {0} )".format(p[2]))
    p[0] = ASTnode('function_definition_empty')
    p[0].value = p[2]
    p[0].child_formals = p[4]
    p[0].child_return = p[7]


def p_function_definition(p):
    '''function_definition : function_definition_empty
                           | function_definition_with_vars'''
    p[0] = p[1]
...
def p_create_empty_node(p):
    '''empty_node : RPAREN RPAREN RPAREN DOUBLEPLUS
                  | '''
    p[0] = ASTnode('empty_node')
...
def p_formals(p):
    ...
    p[0] = ASTnode('formals')
    p[0].child_params = p[1]


def p_single_regex_varIDENT(p):
    '''regexp_COMMA_varIDENT : n_varIDENT'''
    p[0] = ASTnode("var_list")
    p[0].children_vars = [p[1]]
...
def p_function_call(p):
    ...
    print("function_call( {0} )".format(p[1]))
...
def p_atom_const(p):
    '''atom_const : constIDENT'''
    p[0] = ASTnode('atom_const')
    p[0].value = p[1]


def p_atom_other(p):
    '''atom_other : STRING_LITERAL
                  | NUMBER_LITERAL'''
    p[0] = ASTnode('atom')
    p[0].value = p[1]
